	...
	methods
		function d = drop(loc, Rd, Ri, outerI, innerI, vr, rotAng) %Constructor
            ...
            if Ri<0 %flip the droplet to accomodate this.  
                ...
                d.offCenterDist = fsolve(@(x)dropShapeSolver(x, Rd, -1*Ri, 1/vr), -1*Ri); %brad: added 1/vr here to get it to correctly flip droplets with any VR. appears to work without frontend hacks
                d.outerIndex = innerI;
                d.innerIndex = outerI;
                if nargin>6
                    rotAng = rotAng+180;
        ...
        function [fO, fI] = drawDrop(d, N, edgeC, colorO, colorI)  %Draw the edges of the drop.
            if nargin<2
            ...
            if length(d.location)== 2
                ...
                hold on
                fill([x1io, x1I],[x2io, x2I], colorI, 'linewidth', 2, 'edgeColor', edgeC)
                fO = [x1io;x1I;x2io;x2I];
                fI = [x1io;fliplr(x1O);x2io;fliplr(x2O)];
                %polygon creation
            end
       ...
       %2D only:
        function drawDropIndexMatch(d, N)  %Draw the edges of the drop but dashed on upper interface.
            if nargin<2
                N = 1000; %Number of points on edge of drop (increase to make smoother looking)
            end
            Rd = d.radius;
            Ri = d.innerRadius;
            h = d.offCenterDist
