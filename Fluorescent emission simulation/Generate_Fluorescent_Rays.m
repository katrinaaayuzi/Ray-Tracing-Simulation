...
% How many droplets
    numPoints = 5; %In this current version, this is the amount of drops between 0 and 90, which gets doubled.
    contactAngles = linspace(0,90,numPoints); %This must ALWAYS run with a 90 degree drop, as the baseline
    %contactAngles = [10 27 35 50 65 85];
    interestAngles = [90 110 130 150 170]; %These are the droplet contact angles we gather intensity maps for
...
%     emRays = 50000;
    ...
    rayPerCore = 100; %rays sent to each core [always less than total rays]
...
for ii = 1:numDrops
    ...
    save(name,'contactAnglesCorrected','collectedRi','dropLocation','Rd','Ri','no','ni','vr','dropAngle','histStorage','numDrops','environment','logic','planes','nRays','imgDim','intHistStorage','emIntensityStorage','fIntensityStorage')
    forIntensity = false;
    ii
end
toc;
