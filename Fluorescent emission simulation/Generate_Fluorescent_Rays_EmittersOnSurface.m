...
% How many droplets
    CA_start = 0;
    CA_end = 180;
    numPoints = 46; %In this current version, this is the amount of drops between 0 and 90, which gets doubled.
...
name = strcat('EmOnSu-','VR=',num2str(vr),'-',num2str(numPoints),'-numPoints',num2str(contactAngles(1)),'-',num2str(contactAngles(length(contactAngles))),'-',num2str(nRays),'rd.mat');
...
for ii = 1:numPoints
    ...
    %not the vibe
        ...
        %make emitters on the inner surface of HC/W 
%         emitterRadius = 1; 
%         dropletRadius = 45;
%         shrinksize = 1 - emitterRadius/dropletRadius;
        shrinksize = 0.98;
        f(5,:) = f(2,:) * shrinksize; 
        f(6,:) = f(4,:) * shrinksize;  
        
        %Shrink the first and last point of the shrinksize circle to the original coordinates, more accurate for CA>90degrees

        f(5,1) = f(2,1); % first point of original x coordinate
        f(6,1) = f(4,1); % first point of original y coordinate
        f(5,100) = f(2,100); % last point of original x coordinate
        f(6,100) = f(4,100); % last point of original y coordinate
        
        %polyGen = polyshape({f(1,:),f(2,:)},{f(3,:),f(4,:)}); %Generate polygon from points
        polyGen = polyshape({f(5,:),f(2,:)},{f(6,:),f(4,:)}); %Generate polygon from points with emitters on inner side of outsurface
...
%% control droplet time

numPoints = numPoints+1;
luckydrop = 23;
    
%environment = [nm na nc ri minimumAmplitude propagationDistance screenHeight resolution imgDim no];
environment = [1.34 1 1.52 1.34 minimumAmplitude propagationDistance screenHeight resolution imgDim 1.34];
...
logic(4) = true; %were taking the intensity map no matter what

Ri = collectedRi(luckydrop);
theDrop = [];
    
theDrop = drop(dropLocation, Rd, Ri, 1.34, 1.34, vr);
...
dirStorage2 = rDir;
randomRays2 = []; tempRays2 = [];
histStorage2 = hist;
emIntensityStorage2 = emInt;
fIntensityStorage2 = fint;
intHistStorage2 = intHist;

save(name,'contactAngles','collectedRi','dropLocation','Rd','Ri','no','ni','vr','dropAngle','histStorage','histStorage2','numPoints','environment','logic','planes','nRays','imgDim','intHistStorage','emIntensityStorage','fIntensityStorage','intHistStorage2','emIntensityStorage2','fIntensityStorage2')
%save(name,'luckydrop','contactAngles','collectedRi','dropLocation','Rd','Ri','no','ni','vr','dropAngle','histStorage2','numPoints','logic','planes','nRays','imgDim','intHistStorage2','emIntensityStorage2','fIntensityStorage2')
