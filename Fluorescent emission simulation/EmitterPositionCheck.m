...
% How many droplets
    CA_start = 0;
    CA_end = 180;
    numPoints = 21; %In this current version, this is the amount of drops between 0 and 90, which gets doubled.
...
%     emRays = 50000;
    ...
    rayPerCore = 100%00; %rays sent to each core [always less than total rays]
...
%         shrinksize = 1 - emitterRadius/dropletRadius;
        ...
        f(6,:) = f(4,:) * shrinksize;         
        %f(6,1) = f(4,1) * 0.9 - 0.1;
%         for n = 1:length(f(2,:)) %x coordinates shift
%             xshift = f(2,n) - f(5,n);
%             if xshift >= 0
%                 xrealshift = f(2,n) - f(5,n);               
%             else
%                 xrealshift = f(2,n) - f(5,n);                
%             end
%             f(7,n) = f(5,n) + xshift * (1+(1-shrinksize));
%         end  
        
%         for i = 1:length(f(4,:)) %y coordinates shift
%             yshift = f(6,i) - f(4,i);
%             if yshift >= 0
%                 yrealshift = f(6,i) - f(4,i);
%             else
%                 yrealshift = 0; %f(4,i) - f(6,i);           
%             end
%             f(8,i) = f(6,i) - yrealshift;
%         end 

        % Shrink the first and last point of the shrinksize circle to the
        % the original coordinates, more accurate for CA>90degrees
%         if contactAngles >= 90
