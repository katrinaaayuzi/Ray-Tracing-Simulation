...
%% Processing custom INTER DROPLET EMISSION MAPS! 2025 Katrina: for incident light travel through droplets without fluorescent dyes
...
for i = 1:length(intensityStorage)
    ...
    if sum(sum(interest)) > 0
        ...
        %titlestr = ['EM VR=' num2str(vr) '-CA=' num2str(contactAnglesCorrected(i)) '-cont(n=' num2str(environment(1)) ')-HC(n=' num2str(ni) ')-FC(n=' num2str(no) ')'] ;
...
%% emIntensity && Intensity on top of each other, Katrina 2025:????
...
%% Processing custom intensity maps for droplet tracing, Katrina 2025:????
%colorO = [0.5, 0.5, 0.5];
...
for i = 1:length(intensityStorage)
    if sum(sum(intensityStorage{i})) > 0
        ...
        %titlestr = ['CA_d=' num2str(contactAngle(i)) '-cont_n=' num2str(environment(1)) '-HC_n=' num2str(ni) '-FC_n=' num2str(no)] ; % Katrina 2025: what is contact angle, 
        titlestr = ['CA_d=' num2str(contactAngles(i)) '-cont_n=' num2str(environment(1)) '-HC_n=' num2str(ni) '-FC_n=' num2str(no)] ;
        %title(titlestr);
        colorbar
        colormap(inferno)
